...
public:
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::prefix_iterator operator--(int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::prefix_const_iterator operator
		--(int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::prefix_reverse_iterator operator--(
			int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::prefix_const_reverse_iterator operator
		--(int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::infix_iterator operator--(int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::infix_const_iterator operator--(int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::infix_reverse_iterator operator--(
			int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::infix_const_reverse_iterator operator--(
			int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::postfix_const_iterator operator--(
			int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::postfix_reverse_iterator operator--(
			int not_used) noexcept;
	...
	public:
		...
		typename binary_search_tree<tkey, tvalue, compare, tag>::postfix_const_reverse_iterator operator--(
			int not_used) noexcept;
...
binary_search_tree<tkey, tvalue, compare, tag>::prefix_iterator::operator++() & noexcept
{
    ...
    if (_data->left_subtree) {
        ...
        while (parent && (_data == parent->right_subtree || !parent->right_subtree)) {
            _data = parent;
...
binary_search_tree<tkey, tvalue, compare, tag>::postfix_iterator::operator++() & noexcept
{
    ...
    if (!parent || _data == parent->right_subtree || !parent->right_subtree) {
        _data = parent;
    } else {
        _data = parent->right_subtree;
...
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(
        const compare& comp,
        pp_allocator<value_type> alloc,
        logger *logger) : compare(comp), _allocator(alloc), _logger(logger), _root(nullptr) {}
...
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(
        ...
        const compare& cmp,
        pp_allocator<value_type> alloc,
        logger* logger) : compare(cmp), _allocator(alloc), _logger(logger), _root(nullptr)
{
    // && Для того, чтобы в форварде либо перемещать (r-value), либо копировать (l-value)
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(const binary_search_tree &other) : compare(other.compare), _logger(other._logger), _allocator(other._allocator)
{
    if (!other._root) {
        _root = nullptr;
    } else {
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(binary_search_tree &&other) noexcept : compare(std::move(other)), _root(other._root), _logger(other._logger), _allocator(std::move(other._allocator)), _size(other._size)
{
    other._root = nullptr;
...
binary_search_tree<tkey, tvalue, compare, tag>::insert(const value_type& value)
{
    ...
    if (_logger) {
        _logger->log("Вставили ноду", logger::severity::debug);
    }
...
binary_search_tree<tkey, tvalue, compare, tag>::insert_or_assign(const value_type& value)
{
    ...
    if (_logger) {
        _logger->log("Вставили ноду или нашли её", logger::severity::debug);
    }
...
void binary_search_tree<tkey, tvalue, compare, tag>::insert_or_assign(InputIt first, InputIt last)
{
    ...
    if (_logger) {
        _logger->log("Range вставка или поиск завершена", logger::severity::debug);
    }
}
...
binary_search_tree<tkey, tvalue, compare, tag>::emplace_or_assign(Args&&... args)
{
    ...
    if (_logger) {
        _logger->log("эмплейсим или находим ноду", logger::severity::debug);
    }
...
typename binary_search_tree<tkey, tvalue, compare, tag>::infix_iterator binary_search_tree<
    ...
    auto next_temp = ++pos;
	if (_logger) {
		_logger->log("Удалили ноду", logger::severity::debug);
	}


    __detail::bst_impl<tkey, tvalue, compare, tag>::erase(*this, &to_delete._data);

    return infix_iterator(next_temp);
}
