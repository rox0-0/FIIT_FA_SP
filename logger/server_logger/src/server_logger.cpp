...
server_logger::~server_logger() noexcept
{
    ...
    const auto res = _client.Get("/delete?pid=" + pid);
}
    ...
    logger::severity severity) &
{
    ...
    const std::string url = "/log?pid=" + pid +
                          "&sev=" + severity_to_string(severity) +
                          "&message=" + formatted_message;
    const auto res = _client.Get(url);
    return *this;
}
      ...
      _format(std::move(format))
{
    ...
    for (const auto &[sev, stream_info] : _streams) {
        const std::string url = "/init?pid=" + pid +
                              "&sev=" + severity_to_string(sev) +
                              "&path=" + stream_info.first +
                              "&console=" + std::to_string(stream_info.second);
        const auto res = _client.Get(url);
    }
}
